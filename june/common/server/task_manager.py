...
class MongoTaskManager(InMemoryTaskManager):
    ...
    async def upsert_task(self, params: TaskSendParams) -> Task:
        task = await super().upsert_task(params)
        await self._tasks.update_one({"id": task.id}, {"$set": task.model_dump()}, upsert=True)
        return task

    async def _update_store(self, task_id: str, status: TaskStatus, artifacts: list[Artifact] | None = None) -> Task:
        task = await super()._update_store(task_id, status, artifacts)
        await self._tasks.update_one({"id": task.id}, {"$set": task.model_dump()}, upsert=True)
        if artifacts:
            for idx, artifact in enumerate(artifacts):
                doc = artifact.model_dump()
                doc.update({"task_id": task_id, "index": idx})
                await self._artifacts.update_one(
                    {"task_id": task_id, "index": idx}, {"$set": doc}, upsert=True
                )
        return task
