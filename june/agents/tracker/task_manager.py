...
import logging
from common.server.repositories import save_event, get_db
...
class AgentTaskManager(InMemoryTaskManager):
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, TrackerAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    # persist or retrieve timeline
    try:
      user_id = task_send_params.sessionId or "unknown"
      op = data.get("op") if isinstance(data, dict) else None
      if op == "log" and isinstance(data, dict):
        await save_event({"conversation_id": user_id, "entity": data.get("entity"), "data": data.get("data")})
      elif op == "timeline":
        db = await get_db()
        # timeline could be enhanced; here we just ensure collection exists
        await db["audit_logs"].count_documents({"conversation_id": user_id})
    except Exception as e:
