...
class TrackerAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "tracker"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="tracker",
      description="Updates pipeline and returns a searchable timeline.",
      instruction=(
        "Input: JSON {op: 'log'|'timeline', entity: 'job'|'application'|'referral'|'interview'|'offer', data: {...}}. Output: ack or timeline JSON."
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Updating timeline..."}
