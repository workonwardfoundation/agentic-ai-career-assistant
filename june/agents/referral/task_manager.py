...
from .agent import ReferralAgent
import common.server.utils as utils
import logging
from common.server.repositories import save_referrals
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: ReferralAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, ReferralAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    referrals_data = {}
    try:
      referrals_data = json.loads(result)
      parts = [{"type": "data", "data": referrals_data}]
    except:
    ...
    # persist referrals
    try:
      if isinstance(referrals_data, dict) and referrals_data.get("targets"):
        await save_referrals(referrals_data.get("targets"))
    except Exception as e:
      logger.warning(f"Failed to persist referrals: {e}")
