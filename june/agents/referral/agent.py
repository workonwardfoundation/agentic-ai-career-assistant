...
class ReferralAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "referral"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="referral",
      description="Finds insiders and drafts multi-channel outreach variants.",
      instruction=(
        "Input: JSON {job, contacts[], profile_graph}. Output: JSON {targets[], messages[]}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Planning referrals..."}
