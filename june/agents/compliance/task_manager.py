...
import logging
from common.server.repositories import save_compliance_artifact
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: ComplianceAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, ComplianceAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    try:
      ...
      parts = [{"type": "data", "data": comp_data}]
    except:
    ...
    # persist compliance artifact
    try:
      if comp_data:
        await save_compliance_artifact(comp_data)
    except Exception as e:
      logger.warning(f"Failed to persist compliance artifact: {e}")
