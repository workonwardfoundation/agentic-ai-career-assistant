...
from .agent import PartnerAPIAgent
import common.server.utils as utils
import logging
from common.server.repositories import save_partner_api_call
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: PartnerAPIAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, PartnerAPIAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    try:
      if api_data:
        await save_partner_api_call(api_data)
    except Exception as e:
      logger.warning(f"Failed to persist partner API call: {e}")
