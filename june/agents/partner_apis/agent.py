...
class PartnerAPIAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "partner_apis"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="partner_apis",
      description="Integrates with LinkedIn, Indeed, and other job platforms for enhanced functionality.",
      instruction=(
        "Input: JSON {platform: 'linkedin'|'indeed'|'google_jobs', op: 'search'|'apply'|'status', params: {...}}. "
        "Output: JSON with platform-specific results."
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Calling partner API..."}
