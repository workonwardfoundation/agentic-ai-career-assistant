...
class ApplyAgent:
  ...
  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      ...
      name="apply",
      description="Generates application packet and Guided Apply Checklist.",
      instruction=(
        "Input: JSON {job, profile_graph, tailored_docs}. Output: JSON {checklist, attachments[], audit[]}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Generating apply packet..."}
