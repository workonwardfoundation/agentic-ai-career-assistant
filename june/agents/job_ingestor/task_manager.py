...
from .agent import JobIngestorAgent
import common.server.utils as utils
import logging
logger = logging.getLogger(__name__)
from common.server.repositories import save_jobs

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: JobIngestorAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, JobIngestorAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    try:
      ...
      parts = [{"type": "data", "data": jobs_data}]
    except:
