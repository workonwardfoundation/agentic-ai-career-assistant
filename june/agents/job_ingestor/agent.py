...
class JobIngestorAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "job_ingestor"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="job_ingestor",
      description="Normalizes job postings from sources like LinkedIn, Indeed, Google Jobs.",
      instruction=(
        "Given job descriptions or URLs, output a normalized JSON array of jobs with fields: "
        "title, company, location, requirements, apply_url, signals"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str) -> AsyncIterable[Dict[str, Any]]:
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Normalizing jobs..."}
