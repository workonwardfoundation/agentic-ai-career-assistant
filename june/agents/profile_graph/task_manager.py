...
from .agent import ProfileGraphAgent
import common.server.utils as utils
import logging
from common.server.repositories import save_profile_graph, get_db
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: ProfileGraphAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, ProfileGraphAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    try:
      ...
      parts = [{"type": "data", "data": result_json}]
    except:
    ...
    # persist/fetch depending on op
    try:
      op = result_json.get("op") if isinstance(result_json, dict) else None
      user_id = task_send_params.sessionId or "unknown"
      if isinstance(result_json, dict) and result_json.get("graph"):
        await save_profile_graph(user_id, result_json["graph"])
      elif op == "get":
        db = await get_db()
        doc = await db["profile_graphs"].find_one({"user_id": user_id})
        # no direct return mutation; persistence path only
    except Exception as e:
      logger.warning(f"ProfileGraph op failed: {e}")
