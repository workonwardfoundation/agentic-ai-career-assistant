...
class ProfileGraphAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "profile_graph"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="profile_graph",
      description="Maintains canonical profile graph and answers retrieval queries.",
      instruction=(
        "Input: JSON {op: 'upsert'|'get', user_id, graph?}. Output: JSON profile graph or ack"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Processing profile graph operation..."}
