...
class VoiceAgent:
  SUPPORTED_CONTENT_TYPES = ["text", "application/json", "audio/wav", "audio/mp3"]
  ...
  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      ...
      name="voice_agent",
      description="Real-time voice interactions using Twilio Media Streams and Azure STT/TTS.",
      instruction=(
        "Input: JSON {audio_data, session_id, intent?}. Output: JSON {response_text, audio_url?, session_state}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Processing voice input..."}
