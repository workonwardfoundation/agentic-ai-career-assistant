...
class MatcherAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "matcher"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="matcher",
      description="Ranks normalized jobs against a profile graph and emits rationale.",
      instruction=(
        "Input: JSON {profile_graph: {...}, jobs: [...]}. Output: JSON array of {job, score, evidence[]}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Ranking and generating rationales..."}
