...
from .agent import MatcherAgent
import common.server.utils as utils
import logging
logger = logging.getLogger(__name__)
from common.server.repositories import save_matches

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: MatcherAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, MatcherAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    try:
      ...
      parts = [{"type": "data", "data": matches_data}]
    except:
    ...
    try:
      if matches_data:
        await save_matches(matches_data)
    except Exception as e:
      logger.warning(f"Failed to persist matches: {e}")
