...
from .agent import PromptLibraryAgent
import common.server.utils as utils
import logging
from common.server.repositories import save_prompt_version, get_prompt_version, get_ab_test_prompts
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: PromptLibraryAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, PromptLibraryAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    # persist prompt version if creating
    try:
      if isinstance(result_data, dict) and result_data.get("op") == "create":
        await save_prompt_version(result_data)
    except Exception as e:
      logger.warning(f"Failed to persist prompt version: {e}")
