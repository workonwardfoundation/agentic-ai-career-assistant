...
import json
from common.server.repositories import get_prompt_version, get_ab_test_prompts

class PromptLibraryAgent:
  ...
  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      ...
      instruction=(
        "Input: JSON {op: 'get'|'create'|'ab_test'|'rollback', agent, skill, prompt?, version?, test_id?}. "
        "Output: JSON prompt data or operation result."
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Managing prompts..."}
