...
class ResumeTailorAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "resume_tailor"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="resume_tailor",
      description="Generates ATS-safe tailored resumes and cover letters given a profile and JD.",
      instruction=(
        "Input: JSON {profile_graph, jd_text}. Output: JSON {tailored_resume_pdf: uri, cover_letter_pdf: uri, diff_json: {}, rationale_json: {}}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Tailoring resume and cover letter..."}
