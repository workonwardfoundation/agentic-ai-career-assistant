...
from common.server.repositories import save_tailor_result
...
class AgentTaskManager(InMemoryTaskManager):
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, ResumeTailorAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    tailor_data = {}
    try:
      tailor_data = json.loads(result)
      parts = [{"type": "data", "data": tailor_data}]
    except:
    ...
    try:
      if tailor_data:
        await save_tailor_result(tailor_data)
    except Exception as e:
      logger.warning(f"Failed to persist tailor result: {e}")
