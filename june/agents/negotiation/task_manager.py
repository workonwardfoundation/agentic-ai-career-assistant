...
import logging
from common.server.repositories import save_offer_comparison
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: NegotiationAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, NegotiationAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    # persist offer comparison
    try:
      if comp_data:
        await save_offer_comparison(comp_data)
    except Exception as e:
      logger.warning(f"Failed to persist offer comparison: {e}")
