...
class NegotiationAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "negotiation"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="negotiation_coach",
      description="Compares offers and generates counter scripts/talk tracks.",
      instruction=(
        "Input: JSON {offers[], profile_graph}. Output: JSON {comparison, counters[], talk_tracks[]}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Analyzing offers..."}
