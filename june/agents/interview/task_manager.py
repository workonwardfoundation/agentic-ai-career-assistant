...
import logging
from common.server.repositories import save_interview_report
...
class AgentTaskManager(InMemoryTaskManager):
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, InterviewAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    try:
      ...
      parts = [{"type": "data", "data": report_data}]
    except:
    ...
    try:
      if report_data:
        await save_interview_report(report_data)
    except Exception as e:
      logger.warning(f"Failed to persist interview report: {e}")
