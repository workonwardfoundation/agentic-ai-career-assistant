...
class InterviewAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "interview"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="interview_coach",
      description="Conducts mock interviews and emits rubric-scored reports.",
      instruction=(
        "Input: JSON {role, questions[], answers[]?}. Output: JSON {scores{}, highlights[], improvements[], followups[]}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Scoring interview..."}
