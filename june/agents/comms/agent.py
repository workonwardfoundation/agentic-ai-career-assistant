...
class CommsAgent:
  ...
  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      ...
      name="comms",
      description="Classifies inbound user messages and outputs routing directive JSON.",
      instruction=(
        "Input: text. Output: JSON {intent: 'onboard'|'ingest'|'match'|'tailor'|'apply'|'referral'|'outreach'|'tracker', params: {...}}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Classifying message..."}
