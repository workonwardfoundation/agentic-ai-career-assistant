...
import logging
from common.server.repositories import save_event
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: CommsAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, CommsAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _update_store(self, task_id: str, status: TaskStatus, artifacts: list[Artifact] | None = None) -> Task:
    async with self.lock:
      try:
        ...
        logger.error(f"Task {task_id} not not found for updating the task")
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    # persist event
    try:
      await save_event({"conversation_id": task_send_params.sessionId, "routing": data or {"raw": result}})
    except Exception as e:
      logger.warning(f"Failed to persist comms event: {e}")
