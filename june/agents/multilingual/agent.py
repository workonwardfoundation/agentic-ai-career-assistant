...
class MultilingualAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "multilingual"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="multilingual",
      description="Handles translation, localization, and multilingual support for global users.",
      instruction=(
        "Input: JSON {op: 'translate'|'localize'|'detect', text, source_lang?, target_lang?, locale?}. "
        "Output: JSON with translated/localized content."
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Processing translation..."}
