...
class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: MultilingualAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, MultilingualAgent.SUPPORTED_CONTENT_TYPES):
