...
import logging
from common.server.repositories import save_outreach_message
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: OutreachAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, OutreachAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    out_data = {}
    try:
      out_data = json.loads(result)
      parts = [{"type": "data", "data": out_data}]
    except:
    ...
    # persist outreach messages
    try:
      if isinstance(out_data, dict) and out_data.get("sent"):
        for m in out_data.get("sent"):
          await save_outreach_message({"conversation_id": task_send_params.sessionId, **m})
    except Exception as e:
      logger.warning(f"Failed to persist outreach messages: {e}")
