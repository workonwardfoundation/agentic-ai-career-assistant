...
class OutreachAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "outreach"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="outreach",
      description="Sends outreach messages (stub) and tracks responses.",
      instruction=(
        "Input: JSON {targets[], message_template, channel: 'sms'|'email'}. Output: JSON {sent:[], failures:[]}"
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Sending outreach messages..."}
