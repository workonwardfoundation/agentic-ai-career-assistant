...
class OnboardingAgent:
  ...
  def __init__(self):
    ...
    self._user_id = "onboarding"

  def _build_agent(self) -> LlmAgent:
    return LlmAgent(
      model="gemini-2.0-flash-001",
      name="onboarding",
      description="Parses resumes/LinkedIn profile text to build a Profile Graph JSON.",
      instruction=(
        "Input: text or JSON with resume/profile content. Output: JSON profile graph {skills, roles, tenure, achievements, constraints}."
      ),
      tools=[],
    )
  ...
  async def stream(self, query: str, session_id: str):
    ...
    async for event in self._runner.run_async(user_id=self._user_id, session_id=session_id, new_message=content):
      if event.is_final_response():
        ...
        yield {"is_task_complete": False, "updates": "Building profile graph..."}
