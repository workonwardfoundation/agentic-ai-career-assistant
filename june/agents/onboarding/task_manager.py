...
import logging
from common.server.repositories import save_profile_graph, save_profile
logger = logging.getLogger(__name__)

class AgentTaskManager(InMemoryTaskManager):
  def __init__(self, agent: OnboardingAgent):
    super().__init__()
    self.agent = agent
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> Any:
    task_send_params: TaskSendParams = request.params
    if not utils.are_modalities_compatible(task_send_params.acceptedOutputModes, OnboardingAgent.SUPPORTED_CONTENT_TYPES):
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    ...
    try:
      ...
      if graph:
        await save_profile(user_id, {"source": "onboarding"})
        await save_profile_graph(user_id, graph)
    except Exception as e:
      logger.warning(f"Failed to persist profile/profile_graph: {e}")
