...
# Auto-register remote agents from env (comma-separated URLs)
_agent_urls = os.environ.get("A2A_AGENT_URLS", "")
if _agent_urls:
    for _u in [u.strip() for u in _agent_urls.split(",") if u.strip()]:
        try:
            agent_server.manager.register_agent(_u)
        except Exception as e:
            print(f"Failed to auto-register agent {_u}: ", e)
